...
Our game begins it execution in main, where a server is called that determines a random file for a game
and finds all the possible words for that game after the user has determined what kind of game he is
playing -- either single or multiplayer. The server also determines the initial character for the current
game being played. From this point, each game is sent to their ow game room where the chat server controls
the natural flow of the game. Each player has a turn to input their next word and the word_processing file
